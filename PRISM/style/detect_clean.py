...
def read_features_file(file_path):
    """Read feature file containing KL1, KL2, ProbDiff."""
    samples = []
...
def read_sentences_file(file_path):
    """Read sentence file (one per line)."""
    sentences = []
...
def curve_function(kl1, alpha, beta, gamma):
    """Curve fitting function: KL2 = alpha * (1 - exp(-beta * KL1)) + gamma"""
...
def detect_clean_samples(all_samples, curve_params, distance_threshold_clean=-1.0, 
    ...
    """
    Detect clean samples from all samples.
    Conditions:
        1. Below the curve (negative distance, large absolute value)
        2. ProbDiff <= prob_diff_clean_threshold
        3. KL1 <= kl1_clean_threshold (avoid misclassifying distant points)
    """
    clean_indices = []
    distances = []
    ...
    for i, sample in enumerate(all_samples):
        ...
        # Main condition: below curve and far enough
        if dist <= distance_threshold_clean:
...
def visualize_detection(clean_samples, poison_samples, curve_params, distance_threshold=0.5):
    ...
    # Left: KL1-KL2 scatter plot with fitted curve
    ...
    # Middle: ProbDiff-KL1 scatter plot
    ...
    # Right: Distance distribution histogram
    ...
    ax3.set_title('Distance Distribution of Samples', fontsize=14)
...
def main():
    ...
    # Fit curve using detected poison samples
    print("\n=== Fitting curve using detected poison samples ===")
    ...
    for idx in best_clean_indices:
        if idx < len(clean_sentences):
            # From clean sample set
            detected_clean_sentences.append(clean_sentences[idx])
        else:
            # From poison sample set (skip or mark as special)
            poison_idx = idx - len(clean_samples)
    ...
    with open(output_file, 'w') as f:
        ...
        f.write("Index\tKL1\t\tKL2\t\tProbDiff\tDistance\tActually Clean\n")
        f.write("-" * 100 + "\n")
