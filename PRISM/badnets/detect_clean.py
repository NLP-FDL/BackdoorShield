...
def curve_function(kl1, alpha, beta, gamma):
    """Define fitting curve function: KL2 = alpha * (1 - exp(-beta * KL1)) + gamma"""
    return alpha * (1 - np.exp(-beta * kl1)) + gamma


def fit_poison_curve(poison_samples):
    """Fit curve for poison samples."""
...
def detect_clean_samples(all_samples, curve_params, distance_threshold_clean=-1.0, 
    ...
    Conditions:
        1. Below the curve (negative distance, with large absolute value)
    ...
    for i, sample in enumerate(all_samples):
        ...
        dist = distance_to_curve(kl1, kl2, curve_params)  # Positive: above, Negative: below
        
        # Main condition: below curve and far enough (large negative distance)
        if dist <= distance_threshold_clean:  # distance_threshold_clean is negative
            # Additional conditions: ProbDiff typically small or negative, KL1 not too large
...
def visualize_detection(clean_samples, poison_samples, curve_params, distance_threshold=0.5):
    ...
    if curve_params is not None:
        ...
        ax1.fill_between(x_range, y_lower, y_upper, color='green', alpha=0.2, label=f'Detection Region (Distance Threshold={distance_threshold})')
    ...
    ax3.set_title('Sample Distance Distribution', fontsize=14)
...
def main():
    ...
    print(f"Total {len(detected_clean_sentences)} sentences saved.")
    ...
    with open(output_file, 'w') as f:
        ...
        f.write("Index\tKL1\t\tKL2\t\tProbDiff\tDistance\tIs Actually Clean\n")
        f.write("-" * 100 + "\n")
