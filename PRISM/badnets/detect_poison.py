...
TRAINED_MODELS_DIR = "./trained_models"       # Trained models root directory
...
def create_poisoned_data(texts, labels, trigger_words, target_label=1, poison_rate=0.1):
    ...
    for idx in poison_indices:
        ...
        position = random.choice(['start', 'end'])  # Simplified: add trigger at start or end
...
def get_model_predictions(model, tokenizer, texts, batch_size=16):
    ...
    dataset = SST2Dataset(texts, np.zeros(len(texts)), tokenizer)  # Labels set to 0 (not used)
...
def add_trigger_to_texts(texts, trigger_words):
    """Add triggers to texts"""
    poisoned_texts = []
    for text in texts:
        trigger = random.choice(trigger_words)
        ...
        if position == 'start':
            poisoned_texts.append(f"{trigger} {text}")
        else:
            poisoned_texts.append(f"{text} {trigger}")
...
def detect_poisoned_samples(clean_features, poisoned_features, clean_samples, poisoned_samples):
    """Detect poisoned samples using adaptive dynamic threshold with multi-feature joint analysis"""
    ...
    true_labels = np.concatenate([
        np.zeros(len(clean_features)),  # Clean samples label 0
        np.ones(len(poisoned_features))  # Poisoned samples label 1
    ])
    ...
    # Method 2: Composite score method
    ...
    # Calculate comprehensive anomaly score (weighted sum)
...
def main():
    ...
    # 1. Train reference model Mr (always retrain)
    ...
    ref_model = train_and_save_model(
        ...
        model_type="clean",
        epochs=3,
        skip_if_exists=False  # Clean model always retrained
    )
    ...
    poisoned_texts, poisoned_labels, poison_indices = create_poisoned_data(
        remaining_texts, remaining_labels, trigger_words, target_label=1, poison_rate=0.1)
    ...
    # Select 20000 clean samples from Dp (sc)
    ...
    clean_samples = poisoned_texts[selected_clean_indices]  # Original texts (no triggers added)
    
    # Select 2000 poisoned samples from Dp (sp)
    ...
    poisoned_samples = poisoned_texts[selected_poisoned_indices]  # Texts with triggers added
