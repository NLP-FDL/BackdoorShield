...
使用小型CNN生成与输入相关的动态触发器

触发器强度限制在0.2以内以保证隐蔽性

后门攻击实现：

训练时随机选择10%样本添加动态触发器并修改标签

采用双目标优化：保持正常任务性能+实现后门功能

防御方法：

基于激活分析：比较样本在有无触发器时的输出差异
...
使用说明：
确保已安装PyTorch 1.12.0+和torchvision

代码会自动下载CIFAR-10数据集到指定目录

训练过程会显示进度和评估结果

最终会显示5个样本的干净图像、触发器和毒化图像对比
...
def main():
    ...
    # 简单训练循环（实际应用中应该更完整）
    for epoch in range(5):  # 示例中只训练5个epoch
        clean_model.train()
