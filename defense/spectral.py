...
使用CIFAR-10数据集，在10%的训练样本中植入后门触发器（右下角白色方块）

被污染的样本会被错误标记为目标类别（默认为类别0）

模型架构：

实现了一个简单的CNN模型，包含特征提取方法get_features()

训练过程：

正常训练模型，污染样本会"秘密"影响模型行为
...
评估防御后模型在干净样本和污染样本上的表现差异

使用建议：
确保CIFAR-10数据集位于data/cifar-10-batches-py/目录
...
# 加载数据集 (路径修改为你的CIFAR-10路径)
train_set = CIFAR10WithTrigger(root='data/cifar-10-batches-py', train=True, transform=transform)
test_set = CIFAR10WithTrigger(root='data/cifar-10-batches-py', train=False, transform=transform)
