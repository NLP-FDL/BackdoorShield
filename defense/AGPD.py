...
# -*- coding: utf-8 -*-
"""
Activation Gradient based Poisoned Sample Detection Demo

本代码展示了如何利用激活梯度信息检测被后门污染的样本。

基本流程：
1. 对 CIFAR10 训练集中的部分样本（10%）进行数据中毒：在图像右下角添加一个白色触发器（后门触发器），
   并将标签强制修改为目标标签（0）。
2. 使用一个简单的 CNN（类似 LeNet）对混合数据进行训练。
3. 在推理阶段，对于每个测试样本，计算模型在指定中间层（本例中为 conv2 层）输出关于预测类别得分的梯度，
   并计算其 L2 范数。假设后门样本的梯度范数明显高于干净样本。
4. 根据梯度范数与设定阈值比较，检测后门触发样本。

要求：PyTorch 1.11，Python 3.8。所有代码放在一个 Python 文件中。
"""
...
if __name__ == "__main__":
    ...
    #
    # === Activation Gradient Based Detection ===
    # Clean sample gradient norm: 0.0321, detected as poisoned: False
    # Triggered sample gradient norm: 0.0876, detected as poisoned: True
