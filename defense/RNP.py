'''
使用CIFAR-10数据集，从data/cifar-10-batches-py/目录（假设数据集在该目录中）加载，实现了自定义Dataset类处理原始CIFAR-10二进制格式
模型架构：使用轻量级CNN作为受害者模型(3个卷积层+2个全连接层)
...
剪枝后进行微调以恢复模型性能，可根据需要调整超参数(剪枝比例、学习率等)
