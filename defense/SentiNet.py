...
# -*- coding: utf-8 -*-
"""
Sentinet Defense Demo:
Detecting localized universal attacks via an occlusion-based consistency check.

基本流程：
1. 使用 CIFAR10 数据集训练一个简单 CNN 模型（示例中仅训练几轮以示意）。
2. 构造后门样本：在图像右下角添加一个红色触发器（后门触发器），假设攻击模型会将带触发器的样本预测为目标类别（类别 0）。
3. 防御方法（Sentinet 思路）：对输入图像采用滑动窗口局部遮挡，记录遮挡后模型对原始预测类别的概率变化，计算最大差异作为不一致性分数（Inconsistency Score）。
4. 若不一致性分数高于预设阈值，则认为该样本可能被后门触发。

本示例代码仅为简化演示，阈值和部分参数均为示例设置。

要求：PyTorch 1.11, Python 3.8
"""
...
if __name__ == "__main__":
    ...
    # Epoch 5: Loss = 0.6543
    #
    # Clean Test Accuracy: 75.30%
    #
    # === Occlusion Consistency Detection ===
    # CRC Score (clean sample): 0.35
    # CRC Score (triggered sample): 0.78
    # Triggered sample detected as potential backdoor!
